...
public class TarantoolConnectorConfiguration {
    ...
    public static TarantoolConnectorConfiguration from(ConfigurationSource source, TarantoolModuleRefresher refresher) {
        ...
        configuration.clients = instancesConfiguration.getKeys()
                .stream()
                .collect(toImmutableMap(key -> key, key -> TarantoolClientConfiguration.from(instancesConfiguration.getNested(key))));

        configuration.clients.forEach((key, value) ->
