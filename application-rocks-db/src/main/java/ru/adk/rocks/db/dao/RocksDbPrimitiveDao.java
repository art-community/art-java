...
import static ru.adk.rocks.db.constants.RocksDbExceptionMessages.*;
import static ru.adk.rocks.db.constants.RocksDbLoggingMessages.*;
import static ru.adk.rocks.db.module.RocksDbModule.rocksDbModule;
import static ru.adk.rocks.db.module.RocksDbModule.rocksDbModuleState;
...
public interface RocksDbPrimitiveDao {
    static <T> void put(String key, T value, Function<T, byte[]> mapper) {
        ...
        RocksDbPrimitiveDao.put(key.getBytes(), mapper.apply(value));
    }
    ...
    static void put(String key, int value) {
        if (isEmpty(key)) return;
        byte[] keyBytes = key.getBytes();
        byte[] valueBytes = ((Integer) value).toString().getBytes();
        RocksDbPrimitiveDao.put(keyBytes, valueBytes);
    }
    ...
    static void put(String key, double value) {
        if (isEmpty(key)) return;
        byte[] keyBytes = key.getBytes();
        byte[] valueBytes = ((Double) value).toString().getBytes();
        RocksDbPrimitiveDao.put(keyBytes, valueBytes);
    }
    ...
    static void put(String key, long value) {
        if (isEmpty(key)) return;
        byte[] keyBytes = key.getBytes();
        byte[] valueBytes = ((Long) value).toString().getBytes();
        RocksDbPrimitiveDao.put(keyBytes, valueBytes);
    }
    ...
    static <T> void add(String key, T value, Function<T, byte[]> mapper) {
        ...
        RocksDbPrimitiveDao.add(key.getBytes(), mapper.apply(value));
    }
