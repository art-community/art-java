...
import static io.art.rsocket.constants.RsocketModuleConstants.InterceptorTarget.*;
...
public class RsocketLoggingInterceptor implements RSocketInterceptor {
    ...
    private static final Logger logger = logger(RsocketLoggingInterceptor.class);
    private final Supplier<Boolean> enabled;
    private final InterceptorTarget target;

    private RSocketProxy createServerProxy(RSocket rsocket) {
    ...
    private RSocketProxy createClientProxy(RSocket rsocket) {
        return new RSocketProxy(rsocket) {
            private final Logger logger = getLogger();
    ...
    public RSocket apply(RSocket rsocket) {
        return target == SERVER ? createServerProxy(rsocket) : createClientProxy(rsocket);
    }
}
